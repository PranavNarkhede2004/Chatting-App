...
export function ChatSidebar({ 
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      // Push new conversation immediately
      if (onConversationCreated) {
        onConversationCreated(conversation);
      }

      // Also trigger a refresh to pick up server-side changes
      if (onConversationsUpdate) {
        onConversationsUpdate();
      }

      // Focus the newly created conversation
      onConversationSelect(conversation);

      setTimeout(() => setAddUserSuccess(null), 2000);
      
    } catch (error) {
      console.error('Error adding user:', error);
      setAddUserError('User not found or unable to create conversation. Please check the email address.');
    } finally {
